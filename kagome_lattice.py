...
orb = [[0.5,0.0,0.0], [0.0,0.5,0.0], [0.5,0.5,0.0]]
mass = [12.0, 12.0, 12.0]
...
q_path_hex = [[0, 0, 0], [0.5, 0.0, 0], [1.0/3, 1.0/3, 0], [0.0, 0, 0]]
...
###############################################################################
DM = DynamicalMatrix('kagome_test', FC ,alpha0)


DM.get_phonon_band(q_path_hex,q_spacing)
DM.draw_phonon_band()
#DM.make_phband_PROCAR_format(q_grid)

###############################################################################
band_range = [int(i) for i in range(9,15)] ; print '# of bands = ' + str(len(band_range)) + ' Detail bands = ' + str(band_range)
#CTI = ComputeTopologicalInvariants('kagome_test',band_range, q_grid)
